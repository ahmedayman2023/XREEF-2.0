...
import { Loader2, AlertTriangle, X, ImageOff } from "lucide-react";
...
function ImageNode({ data, selected }: NodeProps<ImageNodeType>) {
  ...
  const hasError = Boolean(data.error) || loadFailed;
      ...
      >
        {data.loading ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-1 bg-red-500/[0.04] px-2 relative">
            ...
            {loadFailed && !data.error ? (
              <ImageOff className="w-4 h-4 text-red-400/80" />
            ) : (
